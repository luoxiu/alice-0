...
private extension BagToken {
    
    struct Gen {
        private var token = BagToken(val: 0)

        mutating func next() -> BagToken {
            defer {
                token = BagToken(val: token.val + 1)
            }
            return token
        }
    }
